...
namespace CTS_Analytics.Controllers
{
    ...
    public partial class MnemonicController : Controller
    {
        ...
        public ActionResult Mine_raspozn(int raspoznID, int? page)
        {
            ...
            model.LastTrainDateTime = model.RaspoznTable.RaspoznList.LastOrDefault()?.Date;
        ...
        {
            model.ProdPlan = db.LocalPlansBWithLocationID.Where(l => l.LocationID == model.LocationID).Where(d => d.Date >= fromDate && d.Date <= toDate).Select(p => p.Plan).DefaultIfEmpty().Sum();
            ...
            model.MineName = location.LocationName;
            if (getUserLang(Request.Cookies["lang"]) == "en")
            {
                model.MineName = location.LocationNameEng;
            }

            if (getUserLang(Request.Cookies["lang"]) == "kk")
            {
                model.MineName = location.LocationNameKZ;
            }
        }
        ...
        private RaspoznModel GetRaspoznModel(int recognID, DateTime fromDate, DateTime toDate, int takeCount = int.MaxValue, int offset = 0)
        {
            ...
            var dbvagonNums = GetWagonNumsAndDates(1, fromDate, toDate);
