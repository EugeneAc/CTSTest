...
namespace CTS_Manual_Input.Controllers
{
	...
	public class DictionaryController : Controller
	{
		...
		public ActionResult SkipWeightsIndex(int page = 1)
		{
			...
			{
				Skips = skips,
				SkipWeights = skipWeights.OrderByDescending(t => t.LasEditDateTime).ToPagedList(page, pagesize),
			});
		}
		...
		[HttpPost]
		public ActionResult SkipWeightsAdd(SkipWeight model)
		{
		...
		[HttpPost]
		[DictUserAuthorization]
		public ActionResult SkipWeightsEdit(SkipWeight model)
		{
			if ((model.Weight <= 0) || (model.Weight > 25))
			{
				ModelState.AddModelError("Weight", "Вес скипа должен быть больше 0 и меньше 25 т");
